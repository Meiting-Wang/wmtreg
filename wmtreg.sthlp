...
{p 8 8 2}
{cmd:wmtreg} [{it:namelist}] [{it:{help using:using filename}}] [, {it:{help wmtreg##Options:options}}]
...
{synoptline}
{synopt :{it:namelist}}Names of the stored regression result required(supporting wildcards), input all stored regression results by default{p_end}
...
{synoptline}
{syntab:For all}
{synopt :{opth d:rop(varlist)}}Do not report specified variables{p_end}
{synopt :{opth k:eep(varlist)}}Report specified variables only{p_end}
{synopt :{opth o:rder(varlist)}}Place specified variables at the top of the table{p_end}
{synopt :{opt varl:abels(matchlist)}}Change the display name of the variable in the table{p_end}
{synopt :{opth b:(format:fmt)}}Report ordinary regression coefficients{p_end}
{synopt :{opth beta:(format:fmt)}}Report standardized regression coefficients{p_end}
{synopt :{opth se:(format:fmt)}}Report the standard error below the regression coefficient{p_end}
{synopt :{opth t:(format:fmt)}}Report the t-value below the regression coefficient{p_end}
{synopt :{opth p:(format:fmt)}}Report the p-value below the regression coefficient{p_end}
{synopt :{opt onlyb}}Report the regression coefficient only{p_end}
{synopt :{opt staraux}}Set the star on the standard error or t-value or p-value{p_end}
{synopt :{opt nostar}}Do not report the star{p_end}
{synopt :{opth ind:icate(strings:string)}}Do not report the coefficient of variables, just show whether these variables exist using Yes or No{p_end}
{synopt :{opth s:calars(strings:string)}}{bf:r2}, {bf:ar2}, {bf:pr2}, {bf:aic}, {bf:bic}, {bf:F}, {bf:ll} and {bf:N} are permitted, {bf:r2} and {bf:N} as the default. all of these scalars can be set the format like this: r2(%9.3f){p_end}
{synopt :{opt nonum:bers}}Do not report the serial number of each regression or estimation{p_end}
{synopt :{opt nomt:itles}}Do not report the title of each regression or estimation{p_end}
{synopt :{opth mt:itles(strings:string)}}Customize mtitles. Besides, dependent variables as mtitles in {bf:depvars} and names of regression models as mtitles in {bf:esn}, {bf:depvars} as the default.{p_end}
{synopt :{opth ti:tle(strings:string)}}Set the title for the table, {bf:Regression result} as the default{p_end}
{synopt :{opth mg:roups(strings:string)}}Set up grouping and set group names for these regression models{p_end}
